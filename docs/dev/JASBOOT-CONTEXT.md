...
entero len = longitud_texto(txt)
texto concatenado = concatenar(txt, " jasboot")

# Listas
lista numeros = crear_lista()
lista_agregar(numeros, 10.0)
lista_agregar(numeros, 20.0)
flotante primer = lista_obtener(numeros, 0)

# Mapas
mapa config = mapa_crear()
mapa_poner(config, "nombre", "Jasboot")
texto valor = mapa_obtener(config, "nombre")
```
