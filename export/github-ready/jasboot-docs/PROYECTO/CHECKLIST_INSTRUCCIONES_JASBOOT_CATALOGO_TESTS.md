...
**Referencia técnica:** compilador principal `sdk-dependiente/jas-compiler-c` (`codegen.c`, `sistema_llamadas.c`, `keywords.c`, `parser.c` — `parse_lambda`, `NODE_LAMBDA_DECL`); VM `sdk-dependiente/jasboot-ir` (`ir_format.h`, `vm.c`).
...
| P1 | 18 | `ingresar_texto` / entrada estándar | ✓ | | | [tests/P1_18_ingresar_texto/](../../tests/P1_18_ingresar_texto/) **E**/**R**: sintaxis destino; EOF (`--continuo`); línea >255 chars (`fgets`); batería **R** `r_cu_p18_ctrl_*.jasb` (texto como control: `si`, `mientras`, `seleccionar`, `cuando`, `str_a_entero`, etc.); ver `E/NOTAS_E.md`, `README.md`. |
...
| P3 | 30 | Cadenas: `concatenar`, `longitud` / `longitud_texto`, `dividir` / `dividir_texto`, `minusculas` / `str_minusculas`, `copiar_texto` / `str_copiar` | ✓ | ✓ | ✓ | [tests/P3_30_cadenas_utilidades/](../../tests/P3_30_cadenas_utilidades/) **E**/**R**/**S**: aridad; `concatenar`/`longitud`/`dividir_texto`/`minusculas`; **S** ~3001 tokens tras concatenar `",a"`; `copiar_texto`/`str_copiar` solo **E** (aridad) hasta alinear copia literal en VM (ver `README.md`). |
| P3 | 31 | `extraer_subtexto`, `extraer_antes_de`, `extraer_despues_de` | ✓ | ✓ | ✓ | |
| P3 | 32 | `buscar_en_texto` / `contiene_texto`, `termina_con` (expresión o sentencia según AST) | ✓ | ✓ | ✓ | |
| P3 | 33 | `str_a_entero`, `str_a_flotante`, `str_desde_numero` | ✓ | ✓ | | **E**: texto no numérico |
| P3 | 34 | `ultima_palabra`, `copiar_texto` (sentencias AST si aplican) | ✓ | ✓ | | |
| P3 | 35 | `define_concepto` (NODE_DEFINE_CONCEPTO) | ✓ | | | |
| P3 | 36 | `crear_memoria`, `cerrar_memoria` | ✓ | ✓ | ✓ | **S**: crear/cerrar en bucle |
| P3 | 37 | `recordar` … `con valor` / sin valor | ✓ | ✓ | ✓ | Núcleo JMN |
| P3 | 38 | `buscar` → variable `resultado` | ✓ | ✓ | ✓ | **E**: clave inexistente |
| P3 | 39 | `asociar` (sentencia) y `mem_asociar` (llamada) | ✓ | ✓ | ✓ | |
| P3 | 40 | `aprender` (sentencia) y `aprender` / `aprender_peso` (llamada) | ✓ | ✓ | ✓ | |
| P3 | 41 | `reforzar`, `penalizar` | ✓ | ✓ | ✓ | |
| P3 | 42 | `decae_conexiones` / `decaer_conexiones`, `olvidar_debiles` | ✓ | ✓ | ✓ | |
| P3 | 43 | `consolidar_memoria` / `dormir` / `consolidar` | ✓ | ✓ | ✓ | |
| P3 | 44 | `pensar`, `procesar_texto` | ✓ | ✓ | ✓ | |
...
| P4 | 54 | Archivos: `abrir_archivo` / `fs_abrir`, `cerrar_archivo` / `fs_cerrar`, `leer_linea_archivo` / `fs_leer_linea`, `escribir_archivo` / `fs_escribir`, `fin_archivo`, `existe_archivo`, `listar_archivos` / `fs_listar`, `fs_leer_byte`, `fs_escribir_byte`, `fs_leer_texto` | ✓ | ✓ | ✓ | **E**: permisos, rutas, disco lleno |
...
- Bloques: `principal`, `fin_principal`, `funcion`, `fin_funcion`, `mientras`, `fin_mientras`, `cuando`, `fin_cuando`, `si`, `sino`, `fin_si`, `registro`, `fin_registro`, `concepto`, `fin_concepto`, `hacer`, `usar`
