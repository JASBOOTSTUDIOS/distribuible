# Estrés: N ciclos jbc -e sobre programa que usa imprimir (compilador estable bin\jbc.exe).
$ErrorActionPreference = 'Stop'
$root = Resolve-Path (Join-Path $PSScriptRoot '..\..\..')
$jbc = Join-Path $root 'bin\jbc.exe'
$src = Join-Path $PSScriptRoot '..\R\r_ok_literal_expresion_interpolacion.jasb'
$N = 120
if (-not (Test-Path $jbc)) { throw "No se encontró jbc: $jbc" }
if (-not (Test-Path $src)) { throw "No se encontró: $src" }
